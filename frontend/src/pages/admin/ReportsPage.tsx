...
import { useAdminKPIs, useMonthlyTrend } from '@/hooks/useDashboard'
...
export default function ReportsPage() {
  ...
  const { data: inquiriesData } = useInquiries({ page: 1, per_page: 100 })

  const handleExportExcel = () => {
    if (!inquiriesData?.items?.length) {
      toast.error('No data to export')
      return
    }
    const headers = ['Client Name', 'Phone', 'Event Type', 'Pax', 'Event Date', 'Status', 'Budget', 'Assigned To']
...
${rows.map((r) => `<tr>${r.map((c) => `<td>${c}</td>`).join('')}</tr>`).join('')}
  ...
  const handleExportPDF = () => {
    ...
    const rows = inquiriesData.items.map((inq) => [
      ...
      inq.event_date ?? '-',
      INQUIRY_STATUSES[inq.status as keyof typeof INQUIRY_STATUSES]?.label ?? inq.status,
      inq.budget ? `₹${Number(inq.budget).toLocaleString('en-IN')}` : '-',
      inq.assigned_to ?? '-',
    ])
          ...
          <div className="rounded-xl bg-white p-5 shadow-md">
            ...
            <div className="space-y-2">
              {Object.entries(INQUIRY_STATUSES).map(([key, value]) => {
                ...
                const count = inquiriesData?.items?.filter((inq) => inq.status === key).length ?? 0
                const total = inquiriesData?.total ?? 1
